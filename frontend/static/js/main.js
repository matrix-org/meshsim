...
function update() {
  ...
  let nodes = svg.selectAll(".node").data(data.nodes, n => n.name);
  ...
  t2.merge(t2Enter).text(d => (d.local_echo ? "" : d.name));
}

function updateLinks() {
  let links = svg.selectAll(".link").data(data.links, l => l.id);
  ...
  label
    ...
    .attr("transform", function(l) {
      ...
      return `translate(${(sourceNode.x + targetNode.x) /
        2}, ${7 + (sourceNode.y + targetNode.y) / 2})`;
    });

  let t1 = label.select("text");
  let t1Enter = labelEnter.append("text");

  t1.merge(t1Enter).attr("style", "stroke:white; stroke-width:0.6em");
...
function sendMessages() {
  fetch("/messages", {
    ...
    body: JSON.stringify({
      ...
      node_ids: document.getElementById("node_ids").value.split(","),
      message_size: document.getElementById("message_size").value
    })
  })
    .then(r => {
...
function bootstrap() {
  fetch("/messages/bootstrap", {
    method: "POST",
    headers: { "Content-type": "application/json " },
    body: ""
  })
    .then(r => {
      console.log("Network bootstrapped");
    })
    .catch(function(error) {
      console.log("Request failed", error);
    });
...
function applyDefaults() {
  fetch("/defaults", {
    ...
    body: JSON.stringify({
      ...
      cost_function: document.querySelector(
        'input[name="cost_function"]:checked'
      ).value,
...
var exampleSocket = new WebSocket(
  "ws://" + window.location.host + "/event_notifs"
);
